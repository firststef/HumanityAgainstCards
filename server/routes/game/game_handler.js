...
module.exports = function (app) {
	//**+++++++++++++++++++++++++++++++++++++++++++++++++Game manager */
	app.get("/game_manager/new_id", async (req, res) => {
		try {
	...
	app.get("/game_manager/response", async (req, res) => {
		try {
			...
			res.status(200).send({  msg: `Sucess !`, data: req.headers.data, response: response });
		} catch (e) {
			res.status(417).send({ err: e });
		}
	});
	...
	//**+++++++++++++++++++++++++++++++++++++++++++++++++Game client */
	app.get("/game_client/new", async (req, res) => {
		try {
			if (!req.headers.token) throw `No game token provided!`;
			if (Object.keys(game_client).indexOf(req.headers.token) !== -1) throw `Token "${req.headers.token}" allready registered ! Use /game_client/destroy to destroy a given token!`;

			game_client[token] = new engine.GameClient(req.headers.token);
			res.status(200).send({ msg: `Sucess !`, token: req.headers.token });
		} catch (e) {
			res.status(417).send({ err: e });
		}
	});

	app.get("/game_client/destroy", async (req, res) => {
		try {
			if (!req.headers.token) throw `No game token provided!`;
			if (Object.keys(game_client).indexOf(req.headers.token) == -1) throw `The provided token "${req.headers.token}" was not previously registered !`;
	...
	app.get("/game_client/update", async (req, res) => {
		try {
			if (!req.headers.token) throw `No game token provided!`;
			if (!req.headers.data) throw `No data field provided!`;
			if (Object.keys(game_client).indexOf(req.headers.token) == -1) throw `The provided token "${req.headers.token}" was not previously registered !`;
			await game_client[req.headers.token].update(req.headers.data);
			res.status(200).send({ msg: `Sucess !`, token: req.headers.token, data: req.headers.data });
		} catch (e) {
			res.status(417).send({ err: e });
		}
	});
	app.get("/game_client/get_necesary_data", async (req, res) => {
		try {
			if (!req.headers.token) throw `No game token provided!`;
			if (Object.keys(game_client).indexOf(req.headers.token) == -1) throw `The provided token "${req.headers.token}" was not previously registered !`;
			let data = await game_client[req.headers.token].getNecessaryData();
			if (data === "error") throw `Ackward client error thrown "${data}"`;
			res.status(200).send({ msg: `Sucess !`, token: req.headers.token, response: data });
		} catch (e) {
			res.status(417).send({ err: e });
		}
    });
    app.get("/game_client/put_data", async (req, res) => {
		try {
            if (!req.headers.token) throw `No game token provided!`;
            if (!req.headers.data) throw `No data field provided!`;
            if (Object.keys(game_client).indexOf(req.headers.token) == -1) throw `The provided token "${req.headers.token}" was not previously registered !`;
