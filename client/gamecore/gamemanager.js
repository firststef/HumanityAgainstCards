...
class GameManager {
    constructor(numberOfPlayers, numberOfAIPlayers, playerIDList, roomID) { //should actually be initialized with a gameId, created by looking at the db, assuring it is unique
        ...
        this.timeOut = 10; //seconds to wait for card choice
        this.elapsedTime = 0;
    ...
    startTimer(){
        ...
        this.timerHandle = setInterval(this.timerStep.bind(this), 2000);
    }
    ...
    response(data){//TODO: in server === 'error' should be consistent
        ...
        if(data.header === basedata.RequestHeaders.REQUEST_WAIT_ENDED_PLAYERS){
            console.log('[SERVER] Client requested game status - wait_ended_players');
            //start timer for czar
            if(this.waitEnded_Players === true && this.selectedWhiteCards[0][0] === null){
        ...
        if(data.header === basedata.RequestHeaders.REQUEST_WAIT_ENDED_CZAR){
            console.log('[SERVER] Client requested game status - wait_ended_czar');
            return {
